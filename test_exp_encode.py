...
def test_exp_encode(dataset_path ='./data/', l_inf_r=8/255, poison_amount = 50):
    np.random.seed(random_seed)
    random.seed(random_seed)
    torch.manual_seed(random_seed)
    ...
    acc = evaluate_test_acc(decode, frog_loader, transform_Normalize)
    if acc > 0.99:
    ...
    poison_train_target = poison_image_encode(ori_train, random_poison_idx, encode_cpu, 2, l_inf_r)

    poison_test_target = poison_image_encode_test(test_other_target, encode_cpu, 6, l_inf_r)
    ...
    for epoch in range(200):
        ...
        att = evaluate_test_effect(net, poison_test_loader, transform_Normalize, tar_label)

        acc_clean = evaluate_test_acc(net, frog_test_loader, transform_Normalize)
